...
def load_data(data_dir):
    ...
    logger.info(f"Loaded {len(fake_files)} fake files and {len(real_files)} real files.")
    return files, labels
...
logger.info(f"Extracted features for {len(X_train)} training samples and {len(X_test)} testing samples.")
