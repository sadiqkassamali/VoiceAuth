...
def test_save_metadata(mock_file_metadata):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM file_metadata WHERE uuid = ?", (mock_file_metadata["file_uuid"],)
    )
    result = cursor.fetchone()
    conn.close()
    ...
    assert (
        result[1] == mock_file_metadata["file_path"]
    ), "File path does not match the saved metadata."
    assert (
        result[2] == mock_file_metadata["model_used"]
    ), "Model used does not match the saved metadata."
    assert (
        result[3] == mock_file_metadata["prediction_result"]
    ), "Prediction result does not match the saved metadata."
    assert (
        result[4] == mock_file_metadata["confidence"]
    ), "Confidence does not match the saved metadata."
